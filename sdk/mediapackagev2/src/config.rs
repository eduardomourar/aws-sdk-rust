...
/// Configuration for a aws_sdk_mediapackagev2 service client.
///
...
impl Builder {
    ...
    /// impl aws_sdk_mediapackagev2::config::auth::ResolveAuthScheme for CustomAuthSchemeResolver {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a aws_sdk_mediapackagev2::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    ///     // ...
    ///     .build();
    /// let client = aws_sdk_mediapackagev2::Client::from_conf(config);
    /// ```
    ...
    /// ```no_run
    /// use aws_sdk_mediapackagev2::config::endpoint::{ResolveEndpoint, EndpointFuture, Params, Endpoint};
    /// #[derive(Debug)]
    ...
    /// let config = aws_sdk_mediapackagev2::Config::builder().endpoint_resolver(resolver).build();
    /// let client = aws_sdk_mediapackagev2::Client::from_conf(config);
    /// ```
    ...
    /// # Default Behavior
    ///
    /// When no retry partition is explicitly set, the SDK automatically creates a default retry partition named `mediapackagev2`
    /// (or `mediapackagev2-<region>` if a region is configured).
    /// All MediaPackageV2 clients without an explicit retry partition will share this default partition.
    ///
    /// # Notes
    ///
    ...
    /// ```no_run
    /// use aws_sdk_mediapackagev2::config::Config;
    /// use aws_sdk_mediapackagev2::config::retry::{RetryPartition, TokenBucket};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_mediapackagev2::config::Config;
    /// use aws_sdk_mediapackagev2::config::retry::{ClientRateLimiter, RetryConfig, RetryPartition};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_mediapackagev2::config::IdentityCache;
    ///
    /// let config = aws_sdk_mediapackagev2::Config::builder()
    ...
    /// #    fn meta(&self) -> &aws_sdk_mediapackagev2::error::ErrorMetadata { todo!() }
    /// # }
    ///
    ...
    pub fn build(mut self) -> Config {
        ...
        layer.store_put(::aws_types::SigningName::from_static("mediapackagev2"));
        layer
        ...
        Config {
            config: crate::config::Layer::from(layer.clone())
                .with_name("aws_sdk_mediapackagev2::config::Config")
                .freeze(),
            cloneable: layer,
...
impl ServiceRuntimePlugin {
    pub fn new(_service_config: crate::config::Config) -> Self {
        let config = {
            let mut cfg = ::aws_smithy_types::config_bag::Layer::new("mediapackagev2");
    ...
    pub(crate) fn new(
        ...
        Self {
            config: ::aws_smithy_types::config_bag::Layer::from(layer)
                .with_name("aws_sdk_mediapackagev2::config::ConfigOverrideRuntimePlugin")
                .freeze(),
            components,
        }
    }
        ...
        if input.get_origin("endpoint_url").is_client_config() {
            ...
            builder.set_endpoint_url(
                input
                    .service_config()
                    .and_then(|conf| {
                        conf.load_config(service_config_key("MediaPackageV2", "AWS_ENDPOINT_URL", "endpoint_url"))
...
pub(crate) fn base_client_runtime_plugins(mut config: crate::Config) -> ::aws_smithy_runtime_api::client::runtime_plugin::RuntimePlugins {
    ...
    let default_retry_partition = "mediapackagev2";
    ...
    let scope = "aws-sdk-mediapackagev2";
