...
/// Configuration for a aws_sdk_codebuild service client.
///
...
impl Builder {
    ...
    /// impl aws_sdk_codebuild::config::auth::ResolveAuthScheme for CustomAuthSchemeResolver {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a aws_sdk_codebuild::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    /// ```no_run
    /// use aws_sdk_codebuild::config::endpoint::{ResolveEndpoint, EndpointFuture, Params, Endpoint};
    /// #[derive(Debug)]
    ...
    /// let config = aws_sdk_codebuild::Config::builder().endpoint_resolver(resolver).build();
    /// let client = aws_sdk_codebuild::Client::from_conf(config);
    /// ```
    ...
    /// # Default Behavior
    ///
    /// When no retry partition is explicitly set, the SDK automatically creates a default retry partition named `codebuild`
    /// (or `codebuild-<region>` if a region is configured).
    /// All CodeBuild clients without an explicit retry partition will share this default partition.
    ///
    /// # Notes
    ///
    ...
    /// ```no_run
    /// use aws_sdk_codebuild::config::Config;
    /// use aws_sdk_codebuild::config::retry::{RetryPartition, TokenBucket};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_codebuild::config::Config;
    /// use aws_sdk_codebuild::config::retry::{ClientRateLimiter, RetryConfig, RetryPartition};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_codebuild::config::IdentityCache;
    ///
    /// let config = aws_sdk_codebuild::Config::builder()
    ...
    /// #    fn meta(&self) -> &aws_sdk_codebuild::error::ErrorMetadata { todo!() }
    /// # }
    ///
    ...
    pub fn build(mut self) -> Config {
        ...
        layer.store_put(::aws_types::SigningName::from_static("codebuild"));
        layer
        ...
        Config {
            config: crate::config::Layer::from(layer.clone())
                .with_name("aws_sdk_codebuild::config::Config")
                .freeze(),
            cloneable: layer,
...
impl ServiceRuntimePlugin {
    pub fn new(_service_config: crate::config::Config) -> Self {
        let config = {
            let mut cfg = ::aws_smithy_types::config_bag::Layer::new("CodeBuild_20161006");
    ...
    pub(crate) fn new(
        ...
        Self {
            config: ::aws_smithy_types::config_bag::Layer::from(layer)
                .with_name("aws_sdk_codebuild::config::ConfigOverrideRuntimePlugin")
                .freeze(),
            components,
        }
    }
        ...
        if input.get_origin("endpoint_url").is_client_config() {
            ...
            builder.set_endpoint_url(
                input
                    .service_config()
                    .and_then(|conf| {
                        conf.load_config(service_config_key("CodeBuild", "AWS_ENDPOINT_URL", "endpoint_url"))
...
pub(crate) fn base_client_runtime_plugins(mut config: crate::Config) -> ::aws_smithy_runtime_api::client::runtime_plugin::RuntimePlugins {
    ...
    let default_retry_partition = "codebuild";
