...
/// Configuration for a aws_sdk_rekognition service client.
///
...
impl Builder {
    ...
    /// impl aws_sdk_rekognition::config::auth::ResolveAuthScheme for CustomAuthSchemeResolver {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a aws_sdk_rekognition::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    /// ```no_run
    /// use aws_sdk_rekognition::config::endpoint::{ResolveEndpoint, EndpointFuture, Params, Endpoint};
    /// #[derive(Debug)]
    ...
    /// let config = aws_sdk_rekognition::Config::builder().endpoint_resolver(resolver).build();
    /// let client = aws_sdk_rekognition::Client::from_conf(config);
    /// ```
    ...
    /// # Default Behavior
    ///
    /// When no retry partition is explicitly set, the SDK automatically creates a default retry partition named `rekognition`
    /// (or `rekognition-<region>` if a region is configured).
    /// All Rekognition clients without an explicit retry partition will share this default partition.
    ///
    /// # Notes
    ///
    ...
    /// ```no_run
    /// use aws_sdk_rekognition::config::Config;
    /// use aws_sdk_rekognition::config::retry::{RetryPartition, TokenBucket};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_rekognition::config::Config;
    /// use aws_sdk_rekognition::config::retry::{ClientRateLimiter, RetryConfig, RetryPartition};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_rekognition::config::IdentityCache;
    ///
    /// let config = aws_sdk_rekognition::Config::builder()
    ...
    /// #    fn meta(&self) -> &aws_sdk_rekognition::error::ErrorMetadata { todo!() }
    /// # }
    ///
    ...
    pub fn build(mut self) -> Config {
        ...
        layer.store_put(::aws_types::SigningName::from_static("rekognition"));
        layer
        ...
        Config {
            config: crate::config::Layer::from(layer.clone())
                .with_name("aws_sdk_rekognition::config::Config")
                .freeze(),
            cloneable: layer,
...
impl ServiceRuntimePlugin {
    pub fn new(_service_config: crate::config::Config) -> Self {
        let config = {
            let mut cfg = ::aws_smithy_types::config_bag::Layer::new("RekognitionService");
    ...
    pub(crate) fn new(
        ...
        Self {
            config: ::aws_smithy_types::config_bag::Layer::from(layer)
                .with_name("aws_sdk_rekognition::config::ConfigOverrideRuntimePlugin")
                .freeze(),
            components,
        }
    }
        ...
        if input.get_origin("endpoint_url").is_client_config() {
            ...
            builder.set_endpoint_url(
                input
                    .service_config()
                    .and_then(|conf| {
                        conf.load_config(service_config_key("Rekognition", "AWS_ENDPOINT_URL", "endpoint_url"))
...
pub(crate) fn base_client_runtime_plugins(mut config: crate::Config) -> ::aws_smithy_runtime_api::client::runtime_plugin::RuntimePlugins {
    ...
    let default_retry_partition = "rekognition";
