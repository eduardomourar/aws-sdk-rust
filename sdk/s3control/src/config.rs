...
/// Configuration for a aws_sdk_s3control service client.
///
...
impl Builder {
    ...
    /// impl aws_sdk_s3control::config::auth::ResolveAuthScheme for CustomAuthSchemeResolver {
    ///     fn resolve_auth_scheme<'a>(
    ///         &'a self,
    ///         _params: &'a aws_sdk_s3control::config::auth::Params,
    ///         _cfg: &'a ConfigBag,
    ///         _runtime_components: &'a RuntimeComponents,
    ...
    /// # Default Behavior
    ///
    /// When no retry partition is explicitly set, the SDK automatically creates a default retry partition named `s3control`
    /// (or `s3control-<region>` if a region is configured).
    /// All S3 Control clients without an explicit retry partition will share this default partition.
    ///
    /// # Notes
    ///
    ...
    /// ```no_run
    /// use aws_sdk_s3control::config::Config;
    /// use aws_sdk_s3control::config::retry::{RetryPartition, TokenBucket};
    ///
    ...
    /// ```no_run
    /// use aws_sdk_s3control::config::Config;
    /// use aws_sdk_s3control::config::retry::{ClientRateLimiter, RetryConfig, RetryPartition};
    ///
    ...
    /// #    fn meta(&self) -> &aws_sdk_s3control::error::ErrorMetadata { todo!() }
    /// # }
    ///
    ...
    pub fn build(mut self) -> Config {
        ...
        Config {
            config: crate::config::Layer::from(layer.clone())
                .with_name("aws_sdk_s3control::config::Config")
                .freeze(),
            cloneable: layer,
    ...
    pub(crate) fn new(
        ...
        Self {
            config: ::aws_smithy_types::config_bag::Layer::from(layer)
                .with_name("aws_sdk_s3control::config::ConfigOverrideRuntimePlugin")
                .freeze(),
            components,
        }
    }
